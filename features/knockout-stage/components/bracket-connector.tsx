export function BracketConnector({
  ...
  for (let i = 0; i < pairCount; i++) {
    ...
    paths.push(<line key={`h1-${i}`} x1={startX} y1={y1} x2={midX} y2={y1} className="stroke-slate-400 dark:stroke-slate-500" strokeWidth="2" />)

    // Horizontal from second match to vertical line
    paths.push(<line key={`h2-${i}`} x1={startX} y1={y2} x2={midX} y2={y2} className="stroke-slate-400 dark:stroke-slate-500" strokeWidth="2" />)

    // Vertical line connecting the pair
    paths.push(<line key={`v-${i}`} x1={midX} y1={y1} x2={midX} y2={y2} className="stroke-slate-400 dark:stroke-slate-500" strokeWidth="2" />)

    // Horizontal from vertical line to next round
    paths.push(<line key={`h3-${i}`} x1={midX} y1={midY} x2={endX} y2={midY} className="stroke-slate-400 dark:stroke-slate-500" strokeWidth="2" />)
  }
